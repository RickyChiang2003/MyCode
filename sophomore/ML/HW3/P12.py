...
sigma3 = 0.3162277  # sqrt(0.1)
sigma4 = 0.5477225  # sqrt(0.3)
T = 500
eta = 0.1
#
...
def init_xy(x,y):
    for i in range(N):
        y[i] = (1 if random.randint(-1,2) > 0 or i >= N0 else -1)
        x[i][0] = 1
        if y[i] == 1 and i < N0:
        ...
        if i >= N0:
            x[i][1] = random.gauss(mu = 0, sigma = sigma3)
            x[i][2] = random.gauss(mu = 6, sigma = sigma4)
...
plt.xlabel("Eout(A(D))")
plt.ylabel("Eout(B(D))")
title = "ML hw3 P12 with median = (A:" + str(medianA) + ", B:" + str(medianB) + ")"
plt.title(title)
plt.show()