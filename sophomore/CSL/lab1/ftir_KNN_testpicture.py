...
print('loading...')
knn = cv2.ml.KNearest_load('ftir_mnist_knn.xml')   # 載入模型
print('start...')
if not cap.isOpened():
...
while True:
    ...
    if not ret:
        ...
        break
    # (img.shape) on ricky's computer is (120,160,3)
    img = cv2.resize(img,(160,120))          # 改變影像尺寸，加快處理效率
    x, y, w, h = 50, 50, 50, 50            # 定義擷取數字的區域位置和大小
    ...
    ret, img_num = cv2.threshold(img_num, 95, 255, cv2.THRESH_BINARY_INV)
    output = cv2.cvtColor(img_num, cv2.COLOR_GRAY2BGR)     # 顏色轉成彩色
    img[0:50, 0:50] = output                            # 將轉換後的影像顯示在畫面右上角
